...
(defn interpretar [cod regs-de-act cont-prg pila mapa-regs]
  (let [fetched (cod cont-prg),
    ...
    (case opcode
      ...
      ; NOT: Incrementa cont-prg en 1, quita de la pila un elemento booleano, lo niega y lo coloca al final de la pila.
      NOT (let [nueva_pila (assoc pila (- (count pila) 1) (not (last pila)))]
            (recur cod regs-de-act (inc cont-prg) nueva_pila mapa-regs)
            )

      ; NEG: Incrementa cont-prg en 1, quita de la pila un elemento numerico, le cambia el signo y lo coloca al final de la pila.
      NEG (recur cod regs-de-act (inc cont-prg) (assoc pila (- (count pila) 1) (- (last pila))) mapa-regs)

      ; TOI: Incrementa cont-prg en 1, quita de la pila un elemento numerico, lo convierte a entero y lo coloca al final de la pila.
      TOI (let [nueva_pila (assoc pila (- (count pila) 1) (pasar-a-int (last pila)))]
            (recur cod regs-de-act (inc cont-prg) nueva_pila mapa-regs)
            )

      ; TOF: Incrementa cont-prg en 1, quita de la pila un elemento numerico, lo convierte a punto flotante y lo coloca al final de la pila.
      TOF (let [ nueva_pila (assoc pila (- (count pila) 1) (pasar-a-float (last pila)))]
            (recur cod regs-de-act (inc cont-prg) nueva_pila mapa-regs)
            )
      ...
      ; SQRT: Incrementa cont-prg en 1, quita de la pila un elemento numerico, calcula su raiz cuadrada y la coloca al final de la pila.
      SQRT (let [ nueva_pila (assoc pila (- (count pila) 1) (Math/sqrt (last pila)))]
             (recur cod regs-de-act (inc cont-prg) nueva_pila mapa-regs)
             )

      ; SIN: Incrementa cont-prg en 1, quita de la pila un elemento numerico, calcula su seno y lo coloca al final de la pila.
      SIN (let [nueva_pila (assoc pila (- (count pila) 1) (Math/sin (last pila)))]
            (recur cod regs-de-act (inc cont-prg) nueva_pila mapa-regs)
            )

      ; ATAN: Incrementa cont-prg en 1, quita de la pila un elemento numerico, calcula su arcotangente y la coloca al final de la pila.
      ATAN (let [nueva_pila (assoc pila (- (count pila) 1) (Math/atan (last pila)))]
             (recur cod regs-de-act (inc cont-prg) nueva_pila mapa-regs)
             )

      ; ABS: Incrementa cont-prg en 1, quita de la pila un elemento numerico, calcula su valor absoluto y lo coloca al final de la pila.
      ABS (let [nueva_pila (assoc pila (- (count pila) 1) (Math/abs (last pila)))]
            (recur cod regs-de-act (inc cont-prg) nueva_pila mapa-regs)
            )
      ...
      ; JC: Salto condicional. Quita el ultimo valor de la pila. Si este es true, cambia cont-prg por el valor del argumento. Si no, incrementa cont-prg en 1.
      JC (let [valor (last pila),
               new_prg (if(true? valor) (second fetched)(inc cont-prg))]
           (recur cod regs-de-act new_prg (pop pila) mapa-regs)
           )
      ...
      CAL (let [reg-act-to-add (get mapa-regs (second fetched))]
            (recur cod (conj regs-de-act reg-act-to-add) (second fetched) (conj pila (inc cont-prg)) mapa-regs)
            )
      ...
      ; CHR: Incrementa cont-prg en 1, quita de la pila dos elementos (un string y un indice), selecciona el char del string indicado por el indice y lo coloca al final de la pila.
      CHR (let [
                string-value (first (take-last 2 pila)),
                idx (second (take-last 2 pila)),
                character (nth string-value idx)
                ] (recur cod regs-de-act (inc cont-prg) (conj (vec (drop-last 2 pila)) character) mapa-regs)
                  )
      )
    )
  )
...
; nil
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn add_enter [token]
  (cond
    (= "{" token) (apply str (list "\n" token "\n"))
    (= "}" token) (apply str (list "\n" token "\n"))
    (= ";" token) (apply str (list token "\n"))
    :else token
    )
  )
(defn listar [tokens]
...
(defn add-punto-coma [a b]
  (if (and (= a (symbol "}")) (not (= b (symbol "}"))) (not (= b (symbol ")")))  (not (= b 'else)) (not (= b 'fn)) (not (nil? b)))
    [a (symbol ";")]
    a ))
(defn agregar-ptocoma [tokens]
  (flatten(map add-punto-coma tokens (concat (rest tokens) [nil])))
  )
...
(defn palabra-reservada? [valor]
  (let [palabras_reservadas (set #{'EOF, 'String, 'Write, 'abs, 'as, 'as_str, 'atan, 'bool, 'char, 'chars, 'const,
                                   'else, 'exit, 'expect, 'f64, 'flush, 'fn, 'format!, 'from, 'i64, 'if, 'io,
                                   'let, 'mut, 'new, 'nth, 'parse, 'print!, 'println!, 'process, 'read_line,
                                   'return, 'sin, 'sqrt, 'std, 'stdin, 'stdout, 'to_string, 'trim, 'unwrap, 'use, 'usize,
                                   'while,
                                   } )]
...
(defn identificador? [value]
  (and
    (not (palabra-reservada? value))
    (symbol? value)
    (not (nil? (re-matches #"[a-zA-Z][a-zA-Z]*" (str(get(str value)0))))) )
  )
...
(defn dump [instrucciones]
  (print (apply str (map-indexed instrucciones-numeradas (if (nil? instrucciones) ["nil"] instrucciones) )))
  )
...
(defn get-constant-definition [simbolos]
  (subvec simbolos (get-index simbolos 'const) (+ 6 (get-index simbolos 'const)))
  )
...
(defn cargar-const-en-tabla [ambiente]
  (let [
        ...
        terna [identificador ['const tipo]  valor]]
...
(defn get-return-type [v]
  (let [rustTypes #{'i64 'f64 'String 'bool 'usize 'char}]
      [(last v) (last(last(first (take-last 2 v))))]
    ))

(defn buscar-tipo-de-retorno [ambiente, direccion]
  (let [sub-contexto (get (contexto ambiente) 1)]
    (last(last(filter (fn [v] (= (first v) direccion)) (map get-return-type (filter (fn [v] (= (first (second v)) 'fn)) sub-contexto)))))
    ))
...
(defn generar-ref [ambiente]
  (let [variable (last (simb-ya-parseados ambiente)),
        ...
        sub-contexto (get context 1),
        direccion (last (last (filter (fn [v] (= (first v) variable)) sub-contexto)))]
    (if (= (estado ambiente) :sin-errores)
      (vec[(simb-actual ambiente) (simb-no-parseados-aun ambiente) (simb-ya-parseados ambiente) (estado ambiente) (contexto ambiente) (prox-var ambiente) (conj (bytecode ambiente) ['PUSHADDR direccion]) (mapa-regs-de-act ambiente)])
      ambiente
      )))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn reemplazar [texto reemplazo]
  (cond
    (nil? reemplazo) texto
    (char? reemplazo) (clojure.string/replace texto #"\{" "%c")
    (string? reemplazo) (clojure.string/replace texto #"\{" "%s")
    (int? reemplazo) (clojure.string/replace texto #"\{" "%d")
    (and (float? reemplazo) (== reemplazo (pasar-a-int reemplazo)) ) (clojure.string/replace texto #"\{:?(\.\d+)?" #(str "%" (if (nil? (last %1) ) ".0" (last %1)) "f"))
    (and (float? reemplazo) (not(== reemplazo (pasar-a-int reemplazo))) ) (clojure.string/replace texto #"\{:?(\.\d+)?" #(str "%" (if (nil? (last %1) ) ".3" (last %1)) "f"))
    (and (double? reemplazo) (== reemplazo (pasar-a-int reemplazo)) ) (clojure.string/replace texto #"\{:?(\.\d+)?" #(str "%" (if (nil? (last %1) ) ".0" (last %1)) "f"))
    (and (double? reemplazo) (not(== reemplazo (pasar-a-int reemplazo))) ) (clojure.string/replace texto #"\{:?(\.\d+)?" #(str "%" (if (nil? (last %1) ) ".3" (last %1)) "f"))
    :else texto))


(defn convertir-formato-impresion [argumentos]
   (concat (list(apply str (map reemplazar (clojure.string/split (first argumentos)  #"\}" ) (concat (rest argumentos) [nil])))) (rest argumentos)) )
...
; 0.5
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn dividir [a b] (if (or (float? a) (float? b)) (/ a b) (int (/ a b))))
...
(defn compatibles? [rustType clojureValue]
  (cond
    ...
    (= rustType 'i64) (integer? clojureValue)
    (= rustType 'f64) (float? clojureValue)
    (= rustType 'String) (string? clojureValue)
    (= rustType 'bool) (boolean? clojureValue)
    (= rustType 'usize) (and (integer? clojureValue) (> clojureValue 0))
    (= rustType 'char)  (char? clojureValue)))
